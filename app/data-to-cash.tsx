    ...
    async (phone: string) => {
      ...
      try {
        ...
        const detectedNetwork = networks.find((network) => {
          ...
          console.log(
            `Comparing: API ID="${apiId}" vs Service ID="${serviceId}"`
          );
...
const handleBack = useCallback(() => {
      ...
      [
        { text: "Cancel", style: "cancel" },
        {
          ...
          onPress: () => {
            ...
            // Use router.back() instead of router.push("/(tabs)")
            router.back(); // ✅ Goes back to wherever user came from
          },
        },
      ]
    );
  ...
  <Text className="flex-1 text-[16px] text-red-400 font-manroperegular leading-[20px]">
    To change or create share and sell PIN, dial{" "}
    <Text className="text-[#000000] font-manropesemibold">*347*PIN#</Text>
    {" "}on your phone and then follow the instructions or call customer
    support line{" "}
    <Text className="text-[#000000] font-manropesemibold">300</Text>.
  </Text>
</HStack>
            </VStack>
          </Box>
